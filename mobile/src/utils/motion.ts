/**
 * 动效弹簧物理常量（调研 docs/技术调研/UI设计新潮流调研.md 弹簧参数表）
 *
 * reanimated springify() 默认 damping=120 很"死"，必须显式传参。
 * 物理参数（damping/stiffness）与时长参数互斥——用弹簧时不要混 duration。
 *
 * useReducedMotion 兜底：系统开启「减弱动态效果」时，布局转场退化为无动画（直接跳位），
 * 符合 WCAG 2.3.3 / 系统无障碍约定。调用方用 hooks 版本（useLayoutSpring*）自动获得兜底。
 */
import { useMemo } from "react";
import { LinearTransition, useReducedMotion } from "react-native-reanimated";

/** 通用：列表项重排/布局变化 */
export const SPRING_GENERAL = { damping: 25, stiffness: 250 } as const;
/** 灵敏：长按菜单/快捷操作/键盘避让（跟手优先） */
export const SPRING_SNAPPY = { damping: 15, stiffness: 400 } as const;
/** 温和入场：新消息气泡/卡片出现 */
export const SPRING_GENTLE = { damping: 30, stiffness: 200 } as const;
