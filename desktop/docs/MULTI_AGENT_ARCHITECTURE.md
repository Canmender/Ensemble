...
### 1.1 四层记忆架构（参考腾讯 Agent Memory）

```
┌─────────────────────────────────────────────┐
│ L3: Persona (人格画像)                       │  ← LLM 从 L2 生成长期画像
├─────────────────────────────────────────────┤
│ L2: Scenario Blocks (场景块)                 │  ← LLM 从 L1 按项目/主题组织
├─────────────────────────────────────────────┤
│ L1: Structured Atoms (结构化原子)             │  ← LLM 从 L0 提取事实/偏好/约束
├─────────────────────────────────────────────┤
│ L0: Raw Conversations (原始对话)              │  ← 每次 agent_end 同步写入
└─────────────────────────────────────────────┘
```

**提取流水线**:
```
L0 (原始对话) → L1 (结构化原子) → L2 (场景块) → L3 (人格画像)
     ↑ 同步写入        ↑ 异步提取        ↑ 定时组织     ↑ 全局互斥
     每次对话          批量触发           空闲触发       单线程
```

**热身模式**: 新会话阈值 1→2→4→8→...→N，成熟会话固定阈值
...
### 1.4 关键优化模式（参考腾讯 Agent Memory）

**延迟嵌入 (Deferred Embedding)**:
```
agent_end → 同步写入 L0 元数据 + FTS (毫秒)
         → 异步后台计算嵌入向量 (2-3秒)
         → 不阻塞主流程
```

**上下文分割 (Context Splitting)**:
```
prependContext (动态): L1 相关记忆 → 用户消息前缀 (每轮变化)
appendSystemContext (稳定): L3 人格 + L2 场景 → 系统消息后缀 (可缓存)
```

**混合检索 + RRF 融合**:
```
查询 → BM25 关键词搜索 (jieba 中文分词)
     → 向量语义搜索
     → Reciprocal Rank Fusion (RRF, k=60) 合并排序
```

**Pipeline 热身模式**:
```
新会话: 阈值 1 → 2 → 4 → 8 → ... → 每 N 条 (指数递增)
成熟会话: 固定阈值 + 空闲超时 (600s) 触发
```

**TTL 安全保护**: 删除过期记录时，如果删除比例 > 80%，阻止操作防止误删。

### 1.5 合鸣当前实现 vs 理想状态

| 维度 | 当前实现 | 理想状态 | 参考实现 |
|------|----------|----------|----------|
| 存储 | SQLite FTS5 | FTS5 + sqlite-vec | 腾讯: sqlite-vec 向量表 |
| 检索 | 关键词匹配 | BM25 + 向量 + RRF 混合 | 腾讯: 三策略 + RRF 融合 |
| 分层 | 三层记忆 | L0-L3 四层金字塔 | 腾讯: L0/L1/L2/L3 流水线 |
| 提取 | LLM 定时提取 | 异步 Pipeline + 热身 | 腾讯: 指数递增阈值 |
| 嵌入 | 同步计算 | 延迟嵌入 (异步后台) | 腾讯: fire-and-forget |
| 上下文 | 全部动态注入 | 动态/稳定分离 (缓存优化) | 腾讯: prepend/append 分割 |
| 工具 | memory_write/read/list | + search + forget + persona | 腾讯: 7 种记忆类型 |
| 租户 | 单维度 | 三维隔离 (team/user/agent) | 腾讯: 三列索引 |
