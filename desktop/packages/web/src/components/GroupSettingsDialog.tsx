  ...
  useEffect(() => {
    ...
    (async () => {
      const [aRes, uRes] = await Promise.all([
        ...
        api.get<Array<{ id: string; displayName?: string; username?: string }>>("/auth/users").catch(() => []),
      ]);
      if (cancelled) return;
      setAgents(aRes ?? []);
      setUsers((uRes ?? []).map((u) => ({ id: u.id, name: u.displayName || u.username || u.id })));
    })();
          ...
          <div className="space-y-1.5">
            {memberIds.map((pid) => (
              <div key={pid} className="flex items-center gap-2 rounded-lg border border-border px-3 py-2">
                <div className={cls("flex h-7 w-7 items-center justify-center rounded-full text-xs", isUser(pid) ? "bg-accent/10 text-accent" : "bg-violet-500/10 text-violet-500")}>
                  {isUser(pid) ? "U" : "A"}
                </div>
