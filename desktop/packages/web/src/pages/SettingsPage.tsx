...
function ProviderForm({ initial, onDone }: { initial?: ProviderConfig; onDone: () => void }) {
  ...
  // Auto-generate ID for new providers
  const providerId = initial?.id || ("prov-" + Date.now().toString(36) + Math.random().toString(36).slice(2, 8));
  ...
  async function save() {
    ...
    const finalId = initial?.id || providerId;
    try {
  ...
  async function test() {
    ...
    setTestResult(null);
    try {
      const r = await api.post<{ ok: boolean; message: string }>(`/providers/${initial?.id ?? providerId}/test`);
...
export default function SettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-8 py-8">
      ...
      <div className="mb-4 flex gap-1 rounded-xl bg-muted/10 p-1" role="tablist">
        {tabs.map((t) => (
          <button
            key={t.key}
            role="tab"
      ...
      <button
        ...
        className="mb-4 flex w-full items-center gap-3 rounded-xl border border-border bg-surface px-4 py-3 text-left transition-colors hover:border-primary/40 hover:bg-primary/5"
      >
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary/10 text-primary">
          <Shield className="h-4 w-4" />
        </div>
        <div>
          <div className="text-sm font-medium text-fg">隐私设置</div>
          <div className="text-xs text-muted">管理好友验证、消息权限、个人信息展示</div>
        </div>
      </button>
