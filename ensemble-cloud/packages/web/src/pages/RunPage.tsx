        ...
        {steps.map((it, i) => {
          ...
          if (ev.type === "tool_use") {
            return (
              <div key={i} className="flex gap-2">
                <span className="w-16 shrink-0 text-right text-[10px] text-muted/70">{fmtTime(ev.ts ?? Date.now())}</span>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span className="ml-2 break-all text-muted">{JSON.stringify(ev.input ?? {}).slice(0, 200)}</span>
                </div>
              </div>
            );
          }
          if (ev.type === "tool_result") {
            return (
              <div key={i} className="flex gap-2">
                <span className="w-16 shrink-0 text-right text-[10px] text-muted/70">{fmtTime(ev.ts ?? Date.now())}</span>
                <div className="min-w-0 flex-1 whitespace-pre-wrap break-words pl-6 text-[11px] text-muted">
                  {String(ev.output ?? "").slice(0, 300)}
                </div>
              </div>
            );
          }
