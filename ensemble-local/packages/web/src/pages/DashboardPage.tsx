        ...
        {events.length === 0 ? (
          ...
          <div className="space-y-0.5">
            {events.slice(-60).map((item, i) => {
              ...
              const ts = ev.ts ? new Date(ev.ts).toLocaleTimeString("zh-CN", { hour12: false }) : "";
              if (ev.type === "output") {
                return <div key={i} className={cls("whitespace-pre-wrap break-words", ev.kind === "thinking" ? "text-muted/70 italic" : "text-fg")}><span className="text-[10px] text-muted/50 mr-1">{ts}</span>{ev.text}</div>;
              }
              if (ev.type === "tool_use") {
                return <div key={i} className="break-all text-amber-600 dark:text-amber-400"><span className="text-[10px] text-muted/50 mr-1">{ts}</span>🔧 {ev.tool} {JSON.stringify(ev.input ?? {}).slice(0, 80)}</div>;
              }
              if (ev.type === "status") {
                return <div key={i} className="text-muted"><span className="text-[10px] text-muted/50 mr-1">{ts}</span>· {statusLabel(ev.status)}</div>;
              }
              if (ev.type === "error") {
                return <div key={i} className="whitespace-pre-wrap break-words text-destructive"><span className="text-[10px] text-muted/50 mr-1">{ts}</span>✖ {ev.message}</div>;
              }
              return null;
            })}
          </div>
